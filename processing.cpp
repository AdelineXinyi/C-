...
void compute_energy_matrix(const Image *img, Matrix *energy) {
  ...
  for (int r = 1; r < (*img).height-1; ++r) {
    for (int c = 1; c < (*img).width-1; ++c) {
      ...
      temp = squared_difference(Image_get_pixel(img, r - 1, c),
                                Image_get_pixel(img, r + 1, c)) +
             squared_difference(Image_get_pixel(img, r, c - 1),
                                Image_get_pixel(img, r, c + 1));
      *Matrix_at(energy, r, c) = temp;
      if (max_eng < temp) {
        max_eng = temp;
      }
    }
  }
  Matrix_fill_border(energy, max_eng);
}
...
void find_minimal_vertical_seam(const Matrix *cost, int seam[]) {
  for (int i = Matrix_height(cost)-1; i >=0; --i) {
      ...
      seam[i] = Matrix_column_of_min_value_in_row(
          cost, i, 0, Matrix_width(cost));
      } else {
          if (seam[i+1] == 0){
          seam[i] = Matrix_column_of_min_value_in_row(
              cost, i, seam[i+1], seam[i+1]+2);
          }
          else if (seam[i+1]== Matrix_width(cost)-1){
          seam[i] = Matrix_column_of_min_value_in_row(
              cost, i, seam[i+1]-1, seam[i+1]+1);
          }
          else {
            seam[i] = Matrix_column_of_min_value_in_row(
              cost, i, seam[i+1]-1, seam[i+1]+2);
          }
        }
      }
    }
...
void remove_vertical_seam(Image *img, const int seam[]) {
  ...
  Image_init(aux, Image_width(img)-1, Image_height(img));
   ...
   for (int i = 0; i < Image_height(img); ++i) {
          for (int j = seam[i]; j < Image_width(img)-1; ++j) {
              if (seam[i] != Image_width(img)-1){
